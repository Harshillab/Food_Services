...
{   if(isset($_SESSION['id']))
	...
	$paymentmode=implode(",",$chk);
	  ...
	  if($food_name == ''){
		$name_error = "Foodname should not be empty";
	  }
	  elseif($cost == ''){
		$cost_error = "Cost should not be empty";
	  }
	  elseif($cuisines == ''){
		$cuisines_error = "Cuisines should not be empty";
	  }
	  elseif($img_name == ''){
		$image_error = "Image should not be empty";
	  }
	  else{
...
if(isset($upd_account))
				{
					...
					//echo $emm;
					//echo $add;
					$sql=mysqli_query($con,"select * from tblvendor where fld_name = '$fn'");
					$sql1=mysqli_query($con,"select * from tblvendor where fld_email = '$emm'");

					if(mysqli_num_rows($sql)){
						$name_error = "This User Name is already registered with us";
					  }
					elseif($fn == ''){
						$name_error = "User Name should not be empty";
					}
					elseif(mysqli_num_rows($sql1))
					  {
						$email_error="This Email Id is already registered with us";
					  }
					elseif(!filter_var($emm,FILTER_VALIDATE_EMAIL) || $emm == ''){
...
<nav class="navbar navbar-expand-lg navbar-light bg-dark fixed-top">
	...
	{
	?>
	<a class="navbar-brand" style="color:white; text-decoration:none;"><i class="far fa-user"><?php if(isset($id)) { echo $vr['fld_name']; }?></i></a>
	<?php
	}
	?>
			         ...
			         <!--add Product-->
                        <form action="" method="post" enctype="multipart/form-data">
                                     ...
                                     <label for="food_name">Food Name:</label>
                                            <input type="text" class="form-control" id="food_name" value="<?php if(isset($food_name)) { echo $food_name;}?>" placeholder="Enter Food Name" name="food_name">
                          					<span style="color:red;"><?php if(isset($name_error)){ echo $name_error;} ?></span>
									 </div>
									 
									 
                                     <div class="form-group"><!--cost-->
                                            <label for="cost">Cost :</label>
                                            <input type="number" class="form-control" id="cost"  value="<?php if(isset($cost)) { echo $cost;}?>" placeholder="10000" name="cost" >
                          					<span style="color:red;"><?php if(isset($cost_error)){ echo $cost_error;} ?></span>
                                     </div>
									 
									 
	                                 <div class="form-group"><!--cuisines-->
                                            <label for="cuisines">Cuisines :</label>
                                            <input type="text" class="form-control" id="cuisines" value="<?php if(isset($cuisines)) { echo $cuisines;}?>" placeholder="Enter Cuisines" name="cuisines" >
											<span style="color:red;"><?php if(isset($cuisines_error)){ echo $cuisines_error;} ?></span>                                
									 </div>
	                                ...
	                                <div class="form-group">
										...
										<br><span style="color:red;"><?php if(isset($image_error)){ echo $image_error;} ?></span>                                

									</div>
   
                                    <button type="submit" name="add" class="btn btn-outline-primary">ADD Item</button>
									<br>
			 ...
			 <div class="tab-pane fade" id="accountsettings" role="tabpanel" aria-labelledby="accountsettings-tab">
				...
				?>
					...
					<div class="form-group">
                      ...
                      <input type="text" id="email" value="<?php if(isset($emm)){ echo $emm;}?>" class="form-control" name="emm"/>
					  <span style="color:red;"><?php if(isset($email_error)){ echo $email_error;} ?></span>
					</div>
					<div class="form-group">
                      <label for="address">Address</label>
                      <input type="text" id="address" value="<?php if(isset($ad)){ echo $ad;}?>" class="form-control" name="add"/>
					  <span style="color:red;"><?php if(isset($address_error)){ echo $address_error;} ?></span>
					</div>
					<div class="form-group">
                      <label for="mobile">Mobile Number</label>
                      <input type="text" id="mobile" pattern="[6-9]{1}[0-9]{9}" value="<?php if(isset($mb)){ echo $mb;}?>" class="form-control" name="mob" maxlength="10" />
					  <span style="color:red;"><?php if(isset($mobile_error)){ echo $mobile_error;} ?></span>
					</div>
					
                   <div class="form-group">
                      <label for="pwd">Password:</label>
                     <input type="password" name="pwsd" class="form-control" value="<?php if(isset($psd)){ echo $psd;}?>" id="pwd"/>
					 <span style="color:red;"><?php if(isset($password_error)){ echo $password_error;} ?></span>
				   </div>
				   
				   
 
                  <button type="submit" name="upd_account"  class="btn btn-outline-primary">Update</button>
			...
			<div class="tab-pane fade" id="logo" role="tabpanel" aria-labelledby="logo-tab">
                <div class="container">
				    <form class="form" method="post" enctype="multipart/form-data">
				       <input type="file" name="logo_pic" accept="image/*"/>
			 ...
			 <div class="tab-pane fade " id="status" role="tabpanel" aria-labelledby="status-tab">
			   ...
			   ?>
						<form method="post">
						<td><a href="changestatus.php?order_id=<?php echo $orderrow['fld_order_id']; ?>"><button type="button" name="changestatus" class="btn btn-outline-primary">Update Status</button></a></td>
						</form>
						<tr>
						<?php
					}
