    ...
    if(isset($_SESSION['id']))
{
  ...
  $sql1=mysqli_query($con,"select * from tblvendor where fld_name = '$r_name'");
  $sql1=mysqli_query($con,"select * from tblvendor where fld_password = '$pswd'");

  // $pswd = $_POST['fld_password'];
	// $uppercase = preg_match('@[A-Z]@', $pswd);
	// $lowercase = preg_match('@[a-z]@', $pswd);
	// $number    = preg_match('@[0-9]@', $pswd);
	// $specialChars = preg_match('@[^\w]@', $pswd);
  $paswd = password_hash($pswd,PASSWORD_BCRYPT);
  ...
  // }
    ...
    $token = bin2hex(random_bytes(50));
		$query = mysqli_query($con,"insert into pass_reset1 values('','$email','$token')"); 
...
<div class="middle" style="margin:0px auto; border:1px solid #F8F9FA; margin:0px auto;  width:450px;">
	       ...
	       <div class="tab-pane fade show active" id="register" role="tabpanel" aria-labelledby="home-tab">
			    ...
			    <form action="" method="post" enctype="multipart/form-data">
	                  ...
	                  <div class="form-group">
                          ...
                          <input type="text" class="form-control" id="email" value="<?php if(isset($email)) { echo $email;}?>" placeholder="Enter Email" name="email" />
                     ...
                     <div class="form-group">
                         ...
                         <input type="tel" class="form-control" pattern="[7-9]{1}[0-9]{9}" value="<?php if(isset($mob)) { echo $mob;}?>"id="mob" placeholder="9123456578" name="mob" maxlength = "10" />
	                 ...
	                 <div class="form-group">
                          ...
                          <input type="text" class="form-control" id="add" placeholder="Enter Address" value="<?php if(isset($address)) { echo $address;}?>" name="address">
                          <span style="color:red;"><?php if(isset($address_error)){ echo $address_error;} ?></span>
                     </div>
	                 <div class="form-group">
                          <input type="file"  name="logo">Upload Logo 
                     ...
                     &nbsp;
                     <a href = "vendor_login.php"><button type="button" name="new" class="btn btn-outline-warning">Have an account</button></a>
                </form>
				<br>
			</div>
